...
export const ProviderProfileProvider = ({ children }) => {
  ...
  const [providerType, setProviderTypeState] = useState(() => {
    if (typeof window === 'undefined') return defaultProfileData.serviceType || 'vet';
    return localStorage.getItem('providerType') || defaultProfileData.serviceType || 'vet';
  });
  ...
  const updateProfileData = (newData) =>
    setProfileData((prev) => {
      ...
      if (newData?.serviceType) {
        setProviderType(newData.serviceType);
      }
      return next;
    });

  useEffect(() => {
    if (profileData?.serviceType && profileData.serviceType !== providerType) {
      setProviderType(profileData.serviceType);
    }
  }, [profileData?.serviceType]);

  return (
    <ProviderProfileContext.Provider value={{ profileData, updateProfileData, providerType, setProviderType }}>
      {children}
